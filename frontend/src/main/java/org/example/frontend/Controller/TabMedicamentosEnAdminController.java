...
public class TabMedicamentosEnAdminController implements Initializable{
    ...
    private void buscarMedicamento() {
        ...
        List<Medicamento> resultados = todosLosMedicamentos.stream()
    ...
    public void cargarMedicamentosAsync() {
        ...
        if (btnMostrarTodosLosMedicamentos != null) btnMostrarTodosLosMedicamentos.setDisable(true);
        ...
        Async.run(() -> {
                ...
                medicamentos -> {
                    ...
                    if (btnMostrarTodosLosMedicamentos != null) btnMostrarTodosLosMedicamentos.setDisable(false);
    ...
    public void guardarMedicamentoAsync(Medicamento nuevo) {
        ...
        Async.run(() -> {
                    try {
                        HospitalServiceProxy proxy = new HospitalServiceProxy();
                        if (proxy.conectar()) {
                            SolicitudBackend solicitudVerificar = new SolicitudBackend("VERIFICAR_CODIGO_MEDICAMENTO");
                            solicitudVerificar.agregarParametro("codigo", nuevo.getCodigo());
                            ...
                            SolicitudBackend solicitudInsertar = new SolicitudBackend("INSERTAR_MEDICAMENTO");
                            solicitudInsertar.agregarParametro("medicamento", nuevo);
                ...
                resultado -> {
                    ...
                    if (resultado) {
                        ...
                        mostrarAlerta("Éxito", "Se pudo insertar al medicamento correctamente.");
                    } else {
                ...
                error -> {
                    ...
                    progressMedicamentos.setVisible(false);
                    btnGuardarMedicamento.setDisable(false);
                    mostrarAlerta("Error", "No se pudo guardar: " + error.getMessage());
                });
    }
    public void modificarMedicamentoAsync(Medicamento medicamento) {
        ...
        Async.run(() -> {
                    try {
                        HospitalServiceProxy proxy = new HospitalServiceProxy();
                        if (proxy.conectar()) {
                            SolicitudBackend solicitud = new SolicitudBackend("ACTUALIZAR_MEDICAMENTO");
                            solicitud.agregarParametro("medicamento", medicamento);
                            RespuestaBackend respuesta = (RespuestaBackend) proxy.enviarSolicitud(solicitud);
                            proxy.desconectar();
                ...
                resultado -> {
                    ...
                    cargarMedicamentosAsync();
                    limpiarCamposMedicamentos();
                    mostrarAlerta("Éxito", "Se pudo modificar al medicamento correctamente.");
                },
                error -> {
                    operacionEnProgreso = false;
                    progressMedicamentos.setVisible(false);
                    btnModificarMedicamento.setDisable(false);
                    mostrarAlerta("Error", "No se pudo modificar al medicamento: " + error.getMessage());
                });
    }
    ...
    public void eliminarMedicamentoAsync(Medicamento medicamento) {
        ...
        Async.run(() -> {
                    try {
                        HospitalServiceProxy proxy = new HospitalServiceProxy();
                        if (proxy.conectar()) {
                            SolicitudBackend solicitud = new SolicitudBackend("ELIMINAR_MEDICAMENTO");
                            solicitud.agregarParametro("codigo", medicamento.getCodigo());
                            RespuestaBackend respuesta = (RespuestaBackend) proxy.enviarSolicitud(solicitud);
                            proxy.desconectar();
                            return respuesta.isExito();
                        }
                ...
                resultado -> {
                    ...
                    if (resultado) {
                        cargarMedicamentosAsync();
                        limpiarCamposMedicamentos();
                        mostrarAlerta("Éxito", "Se pudo eliminar al medicamento correctamente.");
                    } else {
                        mostrarAlerta("Error", "No se pudo eliminar al medicamento");
                    }
                },
                error -> {
                    operacionEnProgreso = false;
                    progressMedicamentos.setVisible(false);
                    btnBorrarMedicamento.setDisable(false);
                    mostrarAlerta("Error", "No se pudo eliminar al medicamento: " + error.getMessage());
                });
    }
